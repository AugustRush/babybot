...
_SYSTEM_PROMPT = (
    ...
    "9. 对于“参考仓库/文档/技能做查漏补缺、对比、同步、补齐”这类单一维护目标，默认只保留一个执行 owner：优先一次 dispatch_task 合并所需 resource_ids，不要拆成多个 sibling worker。\n"
    "10. 这类维护目标如需继续补充动作，必须沿着已有执行链串行推进；不要在拿到部分结果后重新开启新的无依赖 worker。\n"
    "\n\n收敛规则（极其重要）：\n"
    ...
    "- 如果 wait_for_tasks / get_task_result 已拿到前序结果，而你要基于该结果继续派生后续子任务，必须显式带上 deps；如果只是整理已有结果，直接 reply_to_user。\n"
    "\n\n资源选择优先级：\n"

...
class HybridMemoryStore:
    ...
    def observe_assistant_message(self, chat_id: str, content: str) -> None:
        ...
        if language == "zh-CN":
            self._insert_or_merge(
                MemoryRecord(
                    ...
                    key="default_language",
                    value="zh-CN",
                    summary="当前对话约定默认中文回复。",
                    tier="soft",
                    scope="chat",
                    scope_id=chat_id,
                    confidence=0.65,
                    last_observed_at=now,
                    tags=("language", "commitment"),
                )
            )
        elif language == "en-US":
            self._insert_or_merge(
        ...
        if style == "concise":
            self._insert_or_merge(
                MemoryRecord(
                    ...
                    key="response_style",
                    value="concise",
                    summary="当前对话约定保持简洁回答。",
                    tier="soft",
                    scope="chat",
                    scope_id=chat_id,
                    confidence=0.62,
                    last_observed_at=now,
                    tags=("style", "commitment"),
                )
            )
        elif style == "detailed":
            self._insert_or_merge(
        ...
        assistant_role = (
            _extract_assistant_role(text)
            or _extract_brief_phrase(text, prefix="作为你的")
            or _extract_brief_phrase(text, prefix="作为")
        )
        if assistant_role:
            self._insert_or_merge(
                MemoryRecord(
                    ...
                    key="assistant_role",
                    value=assistant_role,
                    summary=f"当前助手承诺角色：{assistant_role}。",
                    tier="soft",
                    scope="chat",
                    scope_id=chat_id,
                    confidence=0.66,
                    last_observed_at=now,
                    tags=("role", "commitment"),
                )
            )
